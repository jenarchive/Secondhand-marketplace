...
function mapTestCategoryToForm(testCategory: string): string {
...
export default function EditItemScreen() {
  ...
  const [price, setPrice] = useState(itemData ? String(itemData.price) : '');
  const [category, setCategory] = useState(
    itemData ? mapTestCategoryToForm(itemData.category) : ''
  );
  const [modalVisible, setModalVisible] = useState(false);
  const [focusedInput, setFocusedInput] = useState<string | null>(null);
  const [images, setImages] = useState<string[]>(itemData?.image ? [itemData.image] : []);
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      setImages([...images, ...uniqueNewUris]);
  ...
  const decideBorderColour = (field: string) => {
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.formContainer}>
          ...
          <TextInput
            style={[styles.input, { borderColor: decideBorderColour('title') }]}
            onFocus={() => {
          ...
          <TextInput
            style={[styles.input, { borderColor: decideBorderColour('description') }]}
            onFocus={() => {
          ...
          <TextInput
            style={[styles.input, { borderColor: decideBorderColour('price') }]}
            onFocus={() => {
              ...
              {images.length < 5 && (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    errors.images
                      ? { borderColor: '#FF000080', borderStyle: 'solid' as const, borderWidth: 2 }
                      : { borderColor: '#444', borderStyle: 'dashed' as const, borderWidth: 1 },
                  ]}
                >
          ...
          <TouchableOpacity style={styles.uploadButton} onPress={handleUpdate}>
            <ThemedText style={styles.uploadButtonText}>Update</ThemedText>
          </TouchableOpacity>
        </View>
      </ParallaxScrollView>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  {category === item.name && <Ionicons name="checkmark" size={20} color="#0A84FF" />}
                </TouchableOpacity>
              )}
            />
          </Animated.View>
